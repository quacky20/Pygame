...
class Opponent(Paddle):
    ...
    def get_direction(self):
        self.direction = 1 if self.rect.centery < self.ball.rect.y else -1
...
class Ball(pygame.sprite.Sprite):
    ...
    def wall_collisions(self):
        ...
        if self.rect.left <= 0 or self.rect.right >= WINDOW_WIDTH:
            side = 'player' if self.rect.x < WINDOW_WIDTH / 2 else 'opponent'
    ...
    def reset(self, side):
        ...
        self.direction = pygame.math.Vector2(-1 if side == 'player' else 1, uniform(0.7, 0.8) * choice((-1,1)))
        self.state = 'cooldown'
