...
class UI:
    ...
    def input(self):
        ...
        if self.state == 'general':
            self.general_index['row'] = (self.general_index['row'] + int(keys[pygame.K_DOWN]) - int(keys[pygame.K_UP])) % self.cols
            self.general_index['col'] = (self.general_index['col'] + int(keys[pygame.K_RIGHT]) - int(keys[pygame.K_LEFT])) % self.rows
            ...
            if self.available_monsters:
                self.switch_index = self.switch_index = (self.switch_index + int(keys[pygame.K_DOWN]) - int(keys[pygame.K_UP])) % len(self.available_monsters)
        ...
        if keys[pygame.K_ESCAPE] and self.state != 'general':
            self.attack_index = {'col': 0, 'row': 0}
            self.switch_index = 0
            self.state = 'general'
    ...
    def quad_select(self, index, options):
        ...
        for col in range(self.cols):
            for row in range(self.rows):
                ...
                text_surf = self.font.render(options[col + self.cols*row], True, color)
    ...
    def switch(self):
        ...
        for i in range(len(self.available_monsters)):
            ...
            simple_rect = simple_surf.get_frect(center=(x - 140, y))
    ...
    def stats(self):
        ...
        name_rect = name_surf.get_frect(topleft = rect.topleft + pygame.math.Vector2(rect.width * 0.05, 12))
